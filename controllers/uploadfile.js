...
const Rights = require("../models/permission");
...
const checkFileSize = async (req, res, next) => {
  ...
  let workspace_type = req.query.workspace_type;
...
router.post(
  ...
  upload.any(),
  async (req, res) => {
    console.log(req.body.keyname, "_____data from fromhrtre");
    // upload.any()
  ...
  try {
    ...
    if (id && levels) {
      ...
      for (let file of latestFiles) {
        ...
        let updateLoggs = await loggs.findOne({
          where: {
            file_id: file.id,
          },
          attributes: ["user_id", "createdAt"],
        ...
        //   file.dataValues.update_loggs = {};
        // }
        // let permisison = await Rights.findOne({
        //   where:{
        //     file_id:file.id
        //   },
        //   raw:true
        // })
        // let individual_permission;
        // if (permisison) {
        //   // Store a copy of update_loggs in the variable
        //   individual_permission = { ...update_loggs.dataValues };
        //   file.dataValues.permisison = individual_permission;
        // } else {
        //   file.dataValues.permisison = {};
        // }
        if (updateLoggs) {
          file.dataValues.update_loggs = { ...updateLoggs.dataValues };
        } else {
          file.dataValues.update_loggs = {};
        }

        const permission = await Rights.findOne({
          where: {
            file_id: file.id,
        ...
        if (permission) {
          file.dataValues.permission = { ...permission };
        } else {
          file.dataValues.permission = {};
        }
      }
      ...
      folders.forEach(async (folder) => {
        ...
        const updateLoggs = await loggs.findOne({
          where: {
            folder_id: folder.id,
          },
          attributes: ["user_id", "createdAt"],
        ...
        // }
        if (updateLoggs) {
          folder.dataValues.update_loggs = { ...updateLoggs.dataValues };
        } else {
          folder.dataValues.update_loggs = {};
        }
        ...
        if (permission) {
          folder.dataValues.permission = { ...permission };
        } else {
          folder.dataValues.permission = {};
        }
      });
  ...
  try {
    ...
    async function addSharedInfo(object, sharedData) {
      ...
      sharedData.forEach(async (data) => {
        ...
        if (data.dataValues.shared_by) {
          let userCheck = await User.findOne({
            where: {
              email: data.dataValues.shared_by,
            },
            raw: true,
          });

          if (
            userCheck.user_type == "Admin" &&
            (data.folder_id === object.id || data.file_id === object.id) &&
            data.user_email === userEmail
          ) {
            sharedInfo.shared_by.push(data.dataValues.shared_by);
            sharedInfo.share_with.push(
              data.dataValues.user_email || data.dataValues.guest_email
            );
          }
        }
...
router.post("/filedata", middleware, async (req, res) => {
  try {
    ...
    const chunks = await filesCollection
      .find({
    ...
    let fileContent = Buffer.concat(chunks.map((chunk) => chunk.data.buffer));

    const fileData = await FileUpload.findOne({
      where: {
        filemongo_id: file_id,
      },
      attributes: ["file_type", "user_id", "file_name", "file_type"],
    });
    ...
    if (fileData) {
      ...
      const obj = {
        newdata: fileData.dataValues,
      };
      obj.file_data = fileContent;
...
async function fetchFileContent(filemongo_id) {
  try {
    ...
    const fileContent = Buffer.concat(chunks.map((chunk) => chunk.data.buffer));
    return fileContent;
  } catch (error) {
...
async function createFoldersRecursively(parentId, basePath = ".") {
  ...
  for (const file of folderFiles) {
    const filePath = path.join(folderPath, `${file.file_name}`);
    const fileContent = await fetchFileContent(file.filemongo_id);

    try {
      fs.writeFileSync(filePath, fileContent, "binary");

      if (fs.statSync(filePath).size !== file.file_size) {
        console.error(`File size mismatch for ${filePath}`);
      }
    } catch (error) {
      ...
      FileUpload.findOne({
        ...
        .then(async (foundRow) => {
          if (foundRow) {
            ...
            if (fileDesc) {
              updateData.file_description = fileDesc;
            }
