...
const sendDailyEmail = async (recipients) => {
  try {
    const events = await loggs.findAll({
      where: {
        category: ["Create", "Delete", "Shared", "Auth", "Upload", "Update","Permission"],
        timestamp: {
    ...
    let emailContent =
      ...
      '<th style="background-color: #FFFFCC;">SystemIp</th>' +
    ...
    for (const event of events) {
      ...
      <td style="padding-left: 5px; padding-right: 5px; font-size: 12.6px;">${event.system_ip}</td>
    ...
    if (smtp_details.authentication === "Yes" && smtp_details.security === "TLS") {
    ...
    //     user: smtp_details.from_address,
    ...
    const htmlContent = `
<html>    
<p style="font-family: Calibri;">Dear Admin,</p>
<p style="font-family: Calibri;">The following changes have been made in the last 24 hours:</p>
<p>${emailContent}</p>
...
// cron.schedule("12 21 * * *", fetchDataFromUserDatabase);
      ...
      try {
        ...
        for (const file of allDeletedFiles) {
          ...
          if (fileDeleteTime <= currentTimeInSeconds) {
            if(file.filemongo_id){
              const deletedFile = await filesCollection.deleteOne({
                _id: new ObjectId(file.filemongo_id),
              });
              await chunksCollection.deleteMany({
                files_id: new ObjectId(file.filemongo_id),
              });
              if (deletedFile) {
...
// cron.schedule("11 16 * * *", deactive_user_and_guest);
...
const executePostgresBackup = (backupDir) => {
  ...
  const backupFileName = `${process.env.POSTGRES_DB}_backup_${timestamp}.sql`;
...
const databaseBakup = (backupDir) => {
  try {
    ...
    setTimeout(() => {
      const zipFileName = `DbBackups_${timestamp}.zip`;
      ...
      zipDirectory(
        ...
        () => {
          ...
          cleanUpBackups(backupDir, 4, "DbBackups_", "/");
...
cron.schedule("00 20 * * *", fetchDataFromUserDatabase);
cron.schedule("29 20 * * *", deactive_user_and_guest);
